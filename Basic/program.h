...
public:
	sentence(int lineNumber, Statement *stmt = nullptr, string line = "") :
		lineNumber(lineNumber), stmt(stmt), line(line) {}


	//to sort the set
	bool operator < (const sentence & sts) const {
		return lineNumber < sts.lineNumber;
	}

	//to check if it is the same line
	bool operator == (const sentence & sts) const {
		return lineNumber == sts.lineNumber;
	}
};
